...
This app lives inside the mobile-claw monorepo. When your task involves connection, pairing, relay, bridge, or protocol issues, you **must** check the sibling workspace folders first:

| Repo | Path | Role |
|------|------|------|
| **mobile** (this app) | `.` | Mobile app (RN + Expo) |
| **relay** | `../relay-registry`, `../relay-worker`, `../../packages/relay-shared` | Cloudflare relay + registry |
| **bridge** | `../bridge-cli`, `../../packages/bridge-core`, `../../packages/bridge-runtime` | Local bridge CLI (npm `@flex-lol/mobile-claw`) |
...
All user-visible text in mobile-claw must be internationalized. Hardcoding UI strings is forbidden.
...
mobile-claw uses PostHog for product analytics. Keep instrumentation centralized, reusable, and focused on product decisions.
