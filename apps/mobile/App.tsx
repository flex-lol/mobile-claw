...
function AppContent({
  ...
  // Deep link handler for mobile-claw:// scheme
  useDeepLinkHandler({
    rootNavigationRef,
    gateway,
    mainSessionKey,
  ...
  // Share intent linking config (excludes mobile-claw:// deep links — handled by useDeepLinkHandler)
  const shareIntentLinking = useMemo<LinkingOptions<RootStackParamList>>(() => ({
    ...
    async getInitialURL() {
      ...
      if (url && url.startsWith('mobile-claw://')) return null;
    ...
    subscribe(listener: (url: string) => void) {
      const sub = Linking.addEventListener('url', ({ url }) => {
        if (url.startsWith('mobile-claw://')) return;
