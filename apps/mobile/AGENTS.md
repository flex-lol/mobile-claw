...
mobile-claw is a mobile client for OpenClaw (iOS/Android, React Native + Expo) inside the mobile-claw monorepo.
...
# mobile-claw Ecosystem — Cross-Repository Awareness

mobile-claw now lives in a monorepo. This app is the client-facing frontend; relay and bridge live in sibling workspace folders.
...
```
[mobile-claw App] ←WS→ [Relay] ←WS→ [Bridge CLI] ←WS→ [OpenClaw Gateway]
```
...
3. **Message protocol** — The WS frame format, control frames (`__mobile-claw_relay_control__:` prefix), and `connect`/`challenge` handshake are defined in relay and bridge.
4. **Relay behavior** — Offline message caching, gateway owner lease, heartbeat/alarm logic live in `mobile-claw-relay`.
5. **Bridge lifecycle** — Demand-driven gateway connection, lazy connect/disconnect, service install/uninstall live in `mobile-claw-bridge`.
...
mobile-claw uses PostHog for product analytics. Analytics work must stay centralized and low-noise.
