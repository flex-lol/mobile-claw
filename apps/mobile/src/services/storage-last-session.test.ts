...
describe('StorageService last session key scoping', () => {
  ...
  it('returns the scoped last session key when present', async () => {
    ...
    expect(mockGetItemAsync).toHaveBeenCalledWith(
      'mobile-claw.lastSessionKey.v1.cfg:gw-1',
      expect.any(Object),
    );
  });
  ...
  it('does not fall back to the legacy global key when a scoped key is missing', async () => {
    ...
    expect(mockGetItemAsync).toHaveBeenCalledWith(
      'mobile-claw.lastSessionKey.v1.cfg:gw-2',
      expect.any(Object),
    );
  });
  ...
  it('stores scoped keys without affecting the legacy global slot', async () => {
    ...
    expect(mockSetItemAsync).toHaveBeenCalledWith(
      'mobile-claw.lastSessionKey.v1.cfg:gw-3',
      'agent:main:main',
      expect.any(Object),
    );
  });
  ...
  it('stores and reads the scoped last opened session snapshot', async () => {
    ...
    expect(AsyncStorage.multiSet).toHaveBeenCalledWith(
      [
        ['mobile-claw.lastOpenedSessionSnapshot.v1.cfg:gw-4', JSON.stringify(snapshot)],
        ['mobile-claw.lastOpenedSessionSnapshot.v1.cfg:gw-4::main', JSON.stringify(snapshot)],
      ],
    );
    expect(mockAsyncGetItem).toHaveBeenCalledWith(
      'mobile-claw.lastOpenedSessionSnapshot.v1.cfg:gw-4',
    );
  });
  ...
  it('falls back to the legacy agent-scoped snapshot key when the global slot is missing', async () => {
    ...
    expect(mockAsyncGetItem).toHaveBeenNthCalledWith(
      1,
      'mobile-claw.lastOpenedSessionSnapshot.v1.cfg:gw-5',
    );
    expect(mockAsyncGetItem).toHaveBeenNthCalledWith(
      2,
      'mobile-claw.lastOpenedSessionSnapshot.v1.cfg:gw-5::main',
    );
  });
  ...
  it('stores and reads cached agent identity per gateway scope and agent id', async () => {
    ...
    expect(mockAsyncSetItem).toHaveBeenCalledWith(
      'mobile-claw.cachedAgentIdentity.v1.cfg:gw-6::writer',
