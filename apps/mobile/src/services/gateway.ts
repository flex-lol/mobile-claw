    ...
    try {
      ...
      if (isNonceMismatchError(msg)) {
        this.blockReconnect({
          ...
          message: 'Device authentication nonce mismatch. Please regenerate a new Relay QR code in mobile-claw Bridge.',
          hint: 'Open mobile-claw Bridge and scan a newly generated Relay QR code.',
        });
        this.ws?.close();
        return;
      }
      if (isDeviceSignatureInvalidError(msg)) {
        this.blockReconnect({
          ...
          message: 'Device authentication failed. Reset the mobile-claw app device identity and reconnect.',
      ...
      if (this.isNonRetryableAuthError(msg)) {
        this.blockReconnect({
          ...
          hint: 'Scan a fresh mobile-claw Bridge QR code to refresh this Relay pairing.',
        });
        this.ws?.close();
        return;
      }
