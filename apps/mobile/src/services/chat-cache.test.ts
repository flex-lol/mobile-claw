...
const INDEX_KEY = "mobile-claw.chatCache.index.v2";
...
function makeStorageKey(
  ...
  if (sessionId) {
    return `mobile-claw.chatCache.msgs.${gatewayConfigId}::${agentId}::${sessionKey}::sid:${sessionId}`;
  }
  return `mobile-claw.chatCache.msgs.${gatewayConfigId}::${agentId}::${sessionKey}`;
}
