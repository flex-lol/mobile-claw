...
import { mobile-claw_GITHUB_REPO_URL } from '../../config/app-links';
...
const mobile-claw_IOS_APP_STORE_URL = 'https://apps.apple.com/app/id6759597015';
const mobile-claw_ANDROID_PLAY_STORE_URL = 'https://play.google.com/store/apps/details?id=com.flex-lol.mobile-claw';
...
export function ConfigScreenLayout({ insets, tabBarHeight, controller }: Props): React.JSX.Element {
  ...
  const appVersionLabel = appBuildVersion
    ? t('mobile-claw {{version}} (Build {{build}})', { version: appVersion, build: appBuildVersion })
    : t('mobile-claw {{version}}', { version: appVersion });
  ...
  const handleShareAppPress = useCallback(async () => {
    const shareUrl = Platform.OS === 'ios'
      ? mobile-claw_IOS_APP_STORE_URL
      : mobile-claw_ANDROID_PLAY_STORE_URL;

    try {
      await Share.share({
        title: t('Share mobile-claw'),
        message: t('Try mobile-claw: {{url}}', { url: shareUrl }),
        url: shareUrl,
      });
    } catch {
      ...
      >
        ...
        <View style={styles.card}>
          ...
          >
            ...
            <View style={styles.supportRowText}>
              <Text style={styles.rowLabel}>{t('Share mobile-claw with Friends')}</Text>
            </View>
            <ChevronRight size={16} color={theme.colors.textSubtle} strokeWidth={2} />
          </Pressable>
          ...
          {Platform.OS === 'ios' ? (
            <>
              ...
              >
                ...
                <View style={styles.supportRowText}>
                  <Text style={styles.rowLabel}>{t('Rate mobile-claw')}</Text>
                </View>
                <ChevronRight size={16} color={theme.colors.textSubtle} strokeWidth={2} />
              </Pressable>
        ...
        <View style={styles.card}>
          <Pressable
            onPress={() => {
              void handleOpenExternalUrl(mobile-claw_GITHUB_REPO_URL);
            }}
            style={({ pressed }) => [styles.row, styles.feedbackRow, pressed && styles.rowPressed]}
          >
