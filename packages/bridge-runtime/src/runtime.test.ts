...
import type { PairingConfig } from '@mobile-claw/bridge-core';
...
async function createOpenClawStateDir(config: unknown = {
  ...
  const stateDir = await mkdtemp(join(tmpdir(), 'mobile-claw-bridge-runtime-'));
...
describe('bridge runtime protocol helpers', () => {
  ...
  it('parses relay control frames', () => {
    expect(parseControl('__mobile-claw_relay_control__:{"event":"client_count","count":2}')).toMatchObject({
      event: 'client_count',
      count: 2,
    });
  });
  ...
  it('parses relay control envelopes with request metadata and payload', () => {
    expect(parseControl('__mobile-claw_relay_control__:{"type":"control","event":"bootstrap.request","requestId":"req_bootstrap_1","sourceClientId":"client-a","targetClientId":"gateway-a","payload":{"deviceId":"device-1","count":3}}')).toMatchObject({
  ...
  it('waits for the old gateway socket to close before reconnecting fresh client demand', async () => {
    ...
    relay.message('__mobile-claw_relay_control__:{"event":"client_count","count":1}');
  ...
  it('issues bootstrap tokens for a specific device and replies to the requesting relay client', async () => {
    ...
    relay.message('__mobile-claw_relay_control__:{"type":"control","event":"bootstrap.request","requestId":"req_bootstrap_1","sourceClientId":"client-1","targetClientId":"inst_test","payload":{"deviceId":"device-1","publicKey":"public-key-1","role":"operator","scopes":["operator.write","operator.read"]}}');
    await delay(10);
  ...
  it('returns bootstrap.error when bootstrap.request payload is invalid', async () => {
    ...
    relay.message('__mobile-claw_relay_control__:{"type":"control","event":"bootstrap.request","requestId":"req_bootstrap_invalid","sourceClientId":"client-1","payload":{"deviceId":"device-1","publicKey":"","role":"operator","scopes":[]}}');
    await delay(10);
