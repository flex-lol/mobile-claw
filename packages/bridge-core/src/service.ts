...
const SERVICE_LABEL = 'ai.mobile-claw.bridge.cli';
const SYSTEMD_UNIT_NAME = `${SERVICE_LABEL}.service`;
const WINDOWS_RUN_KEY = 'mobile-clawBridgeCli';
const SERVICE_DIR = join(homedir(), '.mobile-claw');
...
const SERVICE_LAUNCHER_PATH = join(SERVICE_DIR, 'mobile-claw-launcher.sh');
...
const LINUX_CRON_MARKER = '# mobile-claw-bridge-cli';
...
export function buildLinuxSystemdUnit(programArgs: string[], logPath: string, errorLogPath: string): string {
  ...
  return `[Unit]
Description=mobile-claw Bridge CLI
...
function getServicePath(method: ServiceMethod): string {
  switch (process.platform) {
    ...
    case 'win32':
      return 'HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run\\mobile-clawBridgeCli';
    default:
      return '';
  }
}
...
function writeServiceLauncher(input: {
  ...
  const lines = [
    ...
    'fi',
    'echo "node runtime not found for mobile-claw Bridge service" >&2',
    'exit 127',
    '',
  ];
