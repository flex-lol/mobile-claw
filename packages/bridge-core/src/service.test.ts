...
describe('service helpers', () => {
  it('builds POSIX service program args using the CLI script directly', () => {
    expect(getServiceProgramArgs({
      ...
      scriptPath: '/opt/homebrew/bin/mobile-claw',
    }, 'darwin')).toEqual([
      expect.stringMatching(/mobile-claw-launcher\.sh$/),
    ]);
  });
  ...
  it('builds a launch agent plist with keepalive and log paths', () => {
    const plist = buildMacosPlist(
      ['/Users/tester/.mobile-claw/mobile-claw-launcher.sh'],
      '/Users/tester/.mobile-claw/logs/bridge-cli.log',
      '/Users/tester/.mobile-claw/logs/bridge-cli-error.log',
    );

    expect(plist).toContain('<string>ai.mobile-claw.bridge.cli</string>');
    expect(plist).toContain('<key>KeepAlive</key>');
    expect(plist).toContain('/Users/tester/.mobile-claw/logs/bridge-cli.log');
    expect(plist).toContain('/Users/tester/.mobile-claw/mobile-claw-launcher.sh');
  });

  it('builds a systemd user unit that runs the CLI in service mode', () => {
    const unit = buildLinuxSystemdUnit(
      ['/home/tester/.mobile-claw/mobile-claw-launcher.sh'],
      '/home/tester/.mobile-claw/logs/bridge-cli.log',
      '/home/tester/.mobile-claw/logs/bridge-cli-error.log',
    );

    expect(unit).toContain('Description=mobile-claw Bridge CLI');
    expect(unit).toContain('ExecStart="/home/tester/.mobile-claw/mobile-claw-launcher.sh"');
    expect(unit).toContain('Restart=always');
    expect(unit).toContain('StandardOutput=append:/home/tester/.mobile-claw/logs/bridge-cli.log');
  });
  ...
  it('builds a linux cron entry that reboots into the launcher', () => {
    ...
    expect(entry).toContain("'/home/tester/.mobile-claw/mobile-claw-launcher.sh'");
    expect(entry).toContain(">> '/home/tester/.mobile-claw/logs/bridge-cli.log'");
    expect(entry).toContain("2>> '/home/tester/.mobile-claw/logs/bridge-cli-error.log'");
    expect(entry).toContain('# mobile-claw-bridge-cli');
  });

  it('buildLinuxCronEntry correctly single-quote-escapes paths with spaces', () => {
    const entry = buildLinuxCronEntry(
      ["/home/user name/.mobile-claw/mobile-claw-launcher.sh"],
      "/home/user name/.mobile-claw/logs/bridge-cli.log",
      "/home/user name/.mobile-claw/logs/bridge-cli-error.log",
    );

    // spaces inside single-quoted strings are safe without further escaping
    expect(entry).toContain("'/home/user name/.mobile-claw/mobile-claw-launcher.sh'");
    expect(entry).toContain(">> '/home/user name/.mobile-claw/logs/bridge-cli.log'");
    expect(entry).toContain('# mobile-claw-bridge-cli');
  });

  it('buildLinuxCronEntry correctly single-quote-escapes paths with embedded single quotes', () => {
    const entry = buildLinuxCronEntry(
      ["/home/o'brien/.mobile-claw/mobile-claw-launcher.sh"],
      "/home/o'brien/.mobile-claw/logs/bridge-cli.log",
      "/home/o'brien/.mobile-claw/logs/bridge-cli-error.log",
    );

    // embedded ' must be escaped as '"'"'
    expect(entry).toContain(`'/home/o'"'"'brien/.mobile-claw/mobile-claw-launcher.sh'`);
    expect(entry).toContain('# mobile-claw-bridge-cli');
  });
